...
p.addDefiningStateSequence = function(states) {
  ...
  states.forEach(function(state, index, states) {
    ...
    self.addAllowedState(state);
    // Ensure that this state has a transitions object.
    if (!self.transitions[stateKey]) {
      self.transitions[stateKey] = {};
    }

    var nextStateKey;
    if (index === states.length - 1) {
      // This is the end, so it goes to undefined.
      nextStateKey = self.undefinedState.key();
    } else {
    ...
    if (!self.transitions[stateKey][nextStateKey]) {
      self.transitions[stateKey][nextStateKey] = 1;
    } else {
      self.transitions[stateKey][nextStateKey]++;
    }
  });
...
p.initializeTransitionDefinitions = function() {
  ...
  for (stateKey in this.transitions) {
    ...
    newTransitionDefinitions[stateKey].sort(comparator);
  }
